...
type FeaturedPost = Pick<BlogPostSummary, "slug" | "title" | "excerpt" | "coverImage" | "tags">;
		...
		>
			...
			>
				...
				<div className="absolute inset-x-0 bottom-0 p-6 sm:p-8">
					...
					{post.excerpt && (
						<p className="mt-3 line-clamp-2 max-w-md text-sm text-white/80 leading-relaxed">{post.excerpt}</p>
					)}
				</div>
			</Link>
		</motion.article>
...
export function BlogFeaturedSidebar({ posts, className }: BlogFeaturedSidebarProps) {
	return (
		<div className={cn("flex flex-col", className)}>
			<h3 className="mb-5 font-semibold text-gray-400 text-xs uppercase tracking-widest">Featured Posts</h3>
			<div className="flex flex-col gap-5">
				{posts.map((post, index) => (
					...
					>
						<Link className="group flex gap-4" to={`/blog/${post.slug}`}>
