/**
 * The Dakik brandmark — the two interlocking blobs.
 *
 * Single source of truth for the logo SVG, shared by the main-site navbar
 * and every subdomain header (Icons / Bits / Flow). `fill="currentColor"`
 * so the mark inherits the surrounding text color; size it via className
 * (e.g. `h-9 w-auto`). The intrinsic aspect ratio is 83:61.
 */
export function DakikMark({ className }: { className?: string }) {
	return (
		<svg
			aria-hidden="true"
			className={className}
