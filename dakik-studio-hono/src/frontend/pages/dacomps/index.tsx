							...
							<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
								{filtered.map((c) => (
									<article
										...
										key={c.name}
									>
										{/* Whole card navigates to the detail page; the command row
										    below sits above this overlay so copy stays copy-only. */}
										<Link
											aria-label={`View ${c.title ?? c.name}`}
										...
										<div className="min-w-0">
											...
											<h3 className="mt-2 flex items-center gap-1.5 font-bold text-base uppercase tracking-tight">
												{c.title ?? c.name}
												<ArrowUpRight className="size-3.5 text-white/30 opacity-0 transition-opacity group-hover:opacity-100" />
											</h3>
										</div>
										...
										<div className="relative z-10 mt-4 flex items-center gap-2 border border-white/10 bg-black/40 px-3 py-2">
